...
class RBWindow(QMainWindow):
  ...
  def createIcons(self):
    ...
    badlistIcon = QPixmap(resource_path(
        'icons/badlist.png')).scaledToHeight(25, 1)
    ...
    categoryIcons = [raidLeadIcon, guildieIcon, buyerIcon,
                     carryIcon, favoriteIcon, badlistIcon]
  ...
  def PLAYER_INFO(self):
    ...
    self.badlist_combo.setCurrentIndex(0)
    ...
    try:
      ...
      if p_bad:
        self.fav_combo.setCurrentIndex(0)
        self.badlist_combo.setCurrentIndex(1)
  ...
  def countRoster(self, countJson, role):
    ...
    rosterNum += 1
    if role in ("HolyPaladin", "HolyPriest", "Priest", "RestoDruid",
                "RestoShaman", "Shaman"):
      healers += 1
    elif role in ("BearTank", "ProtPaladin", "ProtPaladin1", "Tank"):
      tanks += 1
    elif role in ("AfflictLock", "BeastMaster", "ColdMage", "DemonLock",
                  "DestructoLock", "DisciplinePriest", "HotMage",
                  "Hunter", "Mage", "Marksman", "ShadowPriest",
                  "SmartMage", "Survival", "Warlock"):
      ranged += 1
    elif role in ("ArmsWarrior", "BalanceDruid", "CombatRogue",
                  "ElemShaman", "EnhShaman", "FeralDruid", "FuryWarrior",
                  "MurderRogue", "RetribPaladin", "Rogue", "SneakyRogue",
                  "Warrior"):
      melee += 1
  ...
  def colorMe(self, role, signee):
    newQuery = Query()
    result = newQuery.getThisRoleIcon(role)
    signee = self.setItemToolTip(signee, role)
    if result is not None:
      roleIcon = result[0]
      roleColor = result[1]
      signee.setIcon(QtGui.QIcon(resource_path('{}'.format(roleIcon))))
      signee.setBackground(QtGui.QColor('{}'.format(roleColor)))
      signee.setData(QtCore.Qt.UserRole, role)
    return signee
