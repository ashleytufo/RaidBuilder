...
class Query:
  ...
  def getThisRoleIcon(self, role):
    cursor = self.db.cursor()
    command = "SELECT IconPNG, Color, ClassBox from rb_roles where Name = '{}'; ".format(role)
    result = cursor.execute(command).fetchone()
    self.pyClassLog.debug("%s role icon retrieved from db", (role))
    return result
